...
public class ArRenderer implements GLSurfaceView.Renderer, VuforiaAppRendererControl {
    ...
    public ArRenderer(ArActivity activity,
        ...
        mMovieName = null;
        mCanRequestType = null;
        mSeekPosition = 0;
        mShouldPlayImmediately = false;
        mLostTrackingSince = 0;
        ...
        mVideoPlayerHelper = null;
        mMovieName = "";
        mCanRequestType = ON_TEXTURE_FULLSCREEN;
        mSeekPosition = 0;
        mShouldPlayImmediately = false;
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (isTracking(0)) {
            ...
            mLostTrackingSince = -1;
            if (mVideoPlayerHelper.getStatus() == MEDIA_STATE.READY
                    || mVideoPlayerHelper.getStatus() == MEDIA_STATE.REACHED_END
                    || mVideoPlayerHelper.getStatus() == MEDIA_STATE.PAUSED)
                mVideoPlayerHelper.play(false, mVideoPlayerHelper.getCurrentPosition());
        } else {
            ...
            if (mLostTrackingSince < 0)
                ...
                if ((SystemClock.uptimeMillis() - mLostTrackingSince) > 500) {
                    if (mVideoPlayerHelper != null)
    ...
    private void renderVideo(State state, final float[] projectionMatrix, final float[] temp) {
        // Did we find any trackables this frame?
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++) {
            // Get the trackable:
    ...
    private void checkTextureStatus(float[] projectionMatrix, TrackableResult trackableResult) {
        ...
        if ((currentStatus == MEDIA_STATE.READY)
            ...
            Log.d("currentStatus-vuforia", currentStatus.name());
...
//                }
    ...
    private void checkVideoStatus(float[] projectionMatrix, TrackableResult trackableResult) {
        ...
        // the actual contents
        {
            ...
            GLES20.glVertexAttribPointer(videoPlaybackTexCoordHandle,
                    ...
                    fillBuffer(videoQuadTextureCoordsPerTarget));
            ...
            checkTextureStatus(projectionMatrix, trackableResult);
    ...
    boolean isTapOnScreenInsideTarget(int target, float x, float y) {
        ...
        return (intersection.getData()[0] >= -(targetPositiveDimensions
                .getData()[0]))
    ...
    void setVideoDimensions(int target, float videoWidth, float videoHeight,
        ...
        tempUVMultRes = uvMultMat4f(
                videoQuadTextureCoordsPerTarget[0],
                videoQuadTextureCoordsPerTarget[1],
                videoQuadTextureCoords[0], videoQuadTextureCoords[1], mtx);
        videoQuadTextureCoordsPerTarget[0] = tempUVMultRes[0];
        videoQuadTextureCoordsPerTarget[1] = tempUVMultRes[1];
        tempUVMultRes = uvMultMat4f(
                videoQuadTextureCoordsPerTarget[2],
                videoQuadTextureCoordsPerTarget[3],
                videoQuadTextureCoords[2], videoQuadTextureCoords[3], mtx);
        videoQuadTextureCoordsPerTarget[2] = tempUVMultRes[0];
        videoQuadTextureCoordsPerTarget[3] = tempUVMultRes[1];
        tempUVMultRes = uvMultMat4f(
                videoQuadTextureCoordsPerTarget[4],
                videoQuadTextureCoordsPerTarget[5],
                videoQuadTextureCoords[4], videoQuadTextureCoords[5], mtx);
        videoQuadTextureCoordsPerTarget[4] = tempUVMultRes[0];
        videoQuadTextureCoordsPerTarget[5] = tempUVMultRes[1];
        tempUVMultRes = uvMultMat4f(
                videoQuadTextureCoordsPerTarget[6],
                videoQuadTextureCoordsPerTarget[7],
                videoQuadTextureCoords[6], videoQuadTextureCoords[7], mtx);
        videoQuadTextureCoordsPerTarget[6] = tempUVMultRes[0];
        videoQuadTextureCoordsPerTarget[7] = tempUVMultRes[1];
    }
