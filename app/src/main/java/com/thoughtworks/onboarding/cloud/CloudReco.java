...
import com.thoughtworks.onboarding.AugmentedDisplay.AugmentedRenderer;
...
public class CloudReco extends BaseActivity implements ImageTrackerManager, SampleAppMenuInterface {
    ...
    private static final String kAccessKey = "154a01ae8b006ed397af3631174f0829575e90fe";
    private static final String kSecretKey = "efc010d47c27f49a481185565a0656ff07b1b7cb";
    ...
    private void initApplicationAR() {
        ...
        mRenderer = new AugmentedRenderer(this, vuforiaAppSession);
        ...
        if (statusCode < 0) {
            ...
            if (finder.getResultCount() > 0) {
                ...
                if (result.getTrackingRating() > 0) {
                    ...
                    parseMetadataFromVuforiaCloud(trackable);
    ...
    private void parseMetadataFromVuforiaCloud(Trackable trackable) {
        try {
            final ImageTarget imageTarget = (ImageTarget) trackable;
            Log.i(LOGTAG, "****** CloudRecoRenderer: detected image: " + imageTarget.getMetaData());
            final TargetMetadata targetMetadata = new Gson().fromJson(imageTarget.getMetaData(), TargetMetadata.class);
//            launch(targetMetadata);
//            mRenderer.onDrawFrame(null);
            mRenderer.onSurfaceCreated(null, null);
            /*switch (targetMetadata.mediaType) {
                case VIDEO:
                    launchYoutube(targetMetadata);
                    break;

                case IMAGE:
                    launchImage(targetMetadata);
                    break;
            }*/
        } catch (Exception e) {
            Log.e(LOGTAG, "Failed to parse Trackable: " + e.getMessage());
            e.printStackTrace();
        }
     }

    private void launch(final TargetMetadata targetMetadata) {
        try {
            Intent webIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(targetMetadata.url));
            startActivity(webIntent);
        } catch (ActivityNotFoundException ex) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(CloudReco.this, "Failed to launch target, url: " + targetMetadata.url, Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
