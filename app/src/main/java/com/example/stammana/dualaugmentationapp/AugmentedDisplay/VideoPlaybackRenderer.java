...
import com.example.stammana.dualaugmentationapp.AugmentedDisplay.VideoPlayerHelper.MEDIA_STATE;
import com.example.stammana.dualaugmentationapp.AugmentedDisplay.VideoPlayerHelper.MEDIA_TYPE;
import com.example.stammana.dualaugmentationapp.DisplayType;
...
import com.example.stammana.dualaugmentationapp.utils.KeyFrameShaders;
import com.example.stammana.dualaugmentationapp.utils.SampleUtils;
import com.example.stammana.dualaugmentationapp.utils.Texture;
import com.example.stammana.dualaugmentationapp.utils.VideoPlaybackShaders;
...
public class VideoPlaybackRenderer {
    ...
    VideoPlayerHelper.MEDIA_STATE currentStatus[] = new VideoPlayerHelper.MEDIA_STATE[AugmentedDisplay.NUM_TARGETS];
    // These hold the aspect ratio of both the video and the
    // keyframe
    float videoQuadAspectRatio[] = new float[AugmentedDisplay.NUM_TARGETS];
    float keyframeQuadAspectRatio[] = new float[AugmentedDisplay.NUM_TARGETS];
    ...
    public VideoPlaybackRenderer(AugmentedDisplay activity,
        ...
        this.updateTargetCallback = session;
        videoQuadTextureCoordsPerTarget = new float[AugmentedDisplay.NUM_TARGETS][];
        for (int i = 0; i < AugmentedDisplay.NUM_TARGETS; i++) {
        ...
        mVideoPlayerHelper = new VideoPlayerHelper[AugmentedDisplay.NUM_TARGETS];
        mMovieName = new String[AugmentedDisplay.NUM_TARGETS];
        mCanRequestType = new MEDIA_TYPE[AugmentedDisplay.NUM_TARGETS];
        mSeekPosition = new int[AugmentedDisplay.NUM_TARGETS];
        mShouldPlayImmediately = new boolean[AugmentedDisplay.NUM_TARGETS];
        mLostTrackingSince = new long[AugmentedDisplay.NUM_TARGETS];
    ...
    public void initRendering() {
        Log.d(LOGTAG, "AugmentedDisplay VideoPlaybackRenderer initRendering");
...
//            currentTarget = mActivity.getIndexOfTargetFromTargetName(imageTarget.getName());
            ...
            Pair indexAndType;
            indexAndType = mActivity.getIndexOfTargetFromTargetName(imageTarget.getName());
            int currentTarget = Integer.parseInt(indexAndType.first.toString());
            if (indexAndType.second == DisplayType.IMAGE)
                continue;
...
//                }
            ...
            SampleUtils.checkGLError("AugmentedDisplay renderFrame");
        }
